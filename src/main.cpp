...
uint256 hashGenesisBlock("0xdec71210fd609c48d4228996e6af06bc0e74c3b56be90904f73ec8cb000fdf04");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Switzerlandcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Switzerlandcoin Signed Message:\n";
...
{
    // Switzerlandcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Switzerlandcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Switzerlandcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Switzerlandcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdec71210fd609c48d4228996e6af06bc0e74c3b56be90904f73ec8cb000fdf04");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1399844242;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084536635;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Switzerlandcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SwitzerlandcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SwitzerlandcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    RenameThread("switzerlandcoin-miner");
    ...
    try { loop {
        ...
        printf("Running Switzerlandcoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SwitzerlandcoinMiner terminated\n");
        throw;
    }
}
